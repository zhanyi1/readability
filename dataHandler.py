        ...
        for str in range_file_list:
            if str.find("//") != -1:
                nl_list.append(str)
                    ...
                    and str.find("t1.*, t2.*") == -1 and str.find("inner_query.*") == -1 and str.find("SELECT *")== -1 and str.find("SELECT * ")== -1\
                    and str.find("count(*)")== -1 and str.find("2.0 * ") == -1 and str.find("bodyWeight * 2.0")== -1  and str.find("bodyWeight*-1)")== -1\
                    and str.find(" - 1.0) * ") == -1 and str.find(")*(") == -1 and str.find("/* Here we' go! */") == -1 and str.find(" * 12 )")== -1 \
                    and str.find("select *") == -1 and str.find("/*Notation.findNotation") == -1 and str.find("*=") == -1  and str.find("* 100 )") == -1 :
                nl_list.append(str)
            else:
...
def one_hot_node_type(node_type):
    ...
    hot_dict = {'AssertStmt' : 0, 'BreakStmt': 1, 'ContinueStmt' : 2, 'DoStmt' : 3, 'ExplicitConstructorInvocationStmt' :4,
     'ExpressionStmt' :5 , 'ForEachStmt':6, 'ForStmt':7, 'LabeledStmt':8, 'LocalClassDeclarationStmt':9, 'ReturnStmt':10,
     'SwitchEntry':11 , 'SwitchStmt':12, 'ThrowStmt':13, 'WhileStmt':14, 'MethodDeclaration':15,'AssignExpr':16, 'MethodCallExpr':17
    , 'BinaryExpr':18, 'UnaryExpr':19,
    'VariableDeclarationExpr':20, 'Parameter':21, 'JavadocComment':22, 'LineComment':23, 'BlockComment':24, 'IfStmt':25, 'ElseStmt': 26,
                'ElseIfStmt':27, 'TryStmt': 28, 'FinallyStmt':29, 'CatchStmt':30}
...
def graph_to_input(graph, fileName, target, tokenizer, model):
    ...
    for i in range(len(node_range)):
        ...
        if 'AssignExpr' in node_type[i] \
            ...
            all_node_list.append([i, node_type[i].split(".")[-1], [code_range["beginLine"] - 1, code_range["endLine"] - 1 ], code_list])

        elif 'VariableDeclarationExpr' in node_type[i] \
                or 'Parameter' in node_type[i]:
...
if __name__ == "__main__":
    N_PATHS_AST = "/Users/mojave/Desktop/graph/Data/Neutral"
    R_PATHS_AST = "/Users/mojave/Desktop/graph/Data/Readable"
    U_PATHS_AST = "/Users/mojave/Desktop/graph/Data/Unreadable"
    ...
    for i in range(len(graph_list)):
        ...
        if "Scalabrino15.java" in code_filename_list[i]:

            node_type, raw_code_list, node_embedding_list, edge_list, edge_types, target, node_one_hot_list = graph_to_input(graph_list[i], code_filename_list[i], target_list[i], tokenizer, model)
            nodes_info = []

            graph_raw_code_nodes.append({"graph_name": code_filename_list[i].split("/")[-1], "graph_nodes_codes": raw_code_list, "graph_nodes_type": node_type, "edge_types": edge_types})
    ...
    pkl_data = {"file": graph_raw_code_nodes, "input": graph_input}
    ...
    write_pkl(cpg_dataset[["file", "input"]], "", f"input_logic.pkl")
